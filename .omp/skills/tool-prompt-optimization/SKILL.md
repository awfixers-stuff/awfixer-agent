...
`scripts/probe.ts` routes through `@awfixerai/ai` (`completeSimple`) so model/auth/provider behavior matches production.
...
```ts
import { arkToWireSchema } from "@awfixerai/ai"; // or toolWireSchema(tool)
