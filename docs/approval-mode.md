...
ACP (`agent acp`) uses the same settings resolver as normal agent launches. Global `~/.agent/config.yml` applies, project config for the ACP session `cwd` applies, and any `--config <file>` overlays passed to the ACP server process apply to sessions created by that process.
