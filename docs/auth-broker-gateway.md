...
- **`agent auth-broker serve`** holds the canonical SQLite credential vault, performs OAuth refreshes, and exposes a small REST API (`/v1/snapshot`, `/v1/snapshot/stream`, `/v1/credential/:id/refresh`, `/v1/credential/:id/disable`, `/v1/credential`, `/v1/usage`, `/v1/healthz`).
- **`agent auth-gateway serve`** is a forward-proxy. It accepts OpenAI Chat Completions, Anthropic Messages, OpenAI Responses, and pi-native stream requests, resolves the broker-backed credential, and dispatches through `pi-ai` provider logic. Clients (containerised agent, llm-git, the macOS usage widget, …) never see the access token.
...
```
                ...
                │                                                            │
  developer ──▶ │  ┌──────────────────────────┐    ┌────────────────────┐    │
  laptop /      │  │  agent auth-broker serve   │◀──▶│  SQLite agent.db    │    │
  CI / autoawfixer   │  │  - holds refresh tokens  │    │  (canonical writer)│    │
                ...
                │            ▼                                               │
                │  ┌──────────────────────────┐                              │
                │  │  agent auth-gateway serve  │  RemoteAuthCredentialStore   │
...
```
agent auth-broker serve     [--bind=host:port]                    # boot the broker
agent auth-broker token     [--regenerate] [--json]               # print or rotate the bearer token
agent auth-broker login     [<provider>] [--via=user@host] [--dry-run]
agent auth-broker logout    [<provider>]
agent auth-broker list      [--json]
agent auth-broker import    <file|dir> [--provider=<id>] [--include-disabled] [--dry-run] [--json]
agent auth-broker migrate   --from-local [--include-oauth] [--include-env] [--dry-run] [--json]
agent auth-broker status    [--json]
```
...
- `import <file|dir>` imports CLIProxyAPI-style JSON credentials into the local SQLite store. Maps `type` field → agent provider (`claude → anthropic`, `codex → openai-codex`, `gemini → google-gemini-cli`, `antigravity → google-antigravity`, `gemini-cli → google-gemini-cli`).
...
```
agent auth-gateway serve   [--bind=host:port] [--no-auth]
agent auth-gateway token   [--regenerate] [--json]
agent auth-gateway status  [--json]
agent auth-gateway check   [--strict] [--json]
```
...
The model id is read from the top-level `model` field for foreign wire formats and from the pi-native request body for `/v1/pi/stream`. The gateway picks the first bundled `Model<Api>` matching that id, parses the inbound wire format into an agent `Context`, resolves the provider credential from broker-backed `AuthStorage`, dispatches through `streamSimple()`, and re-encodes the result to the inbound format (SSE for streamed responses).
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `OMP_AUTH_BROKER_URL`   | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`). Selecting this puts the client in broker mode — local SQLite is bypassed. | Any time the agent client should resolve credentials through a broker (and required by `agent auth-gateway serve`).           |
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE`  | Path override for the encrypted local snapshot cache. Default `~/.agent/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). | Optional in broker mode. |
...
| Path                              | Owner                                                | Mode                          |
| --------------------------------- | ---------------------------------------------------- | ----------------------------- |
| `<config-dir>/auth-broker.token`  | `agent auth-broker serve` (created at first start)     | `0600` in a `0700` parent dir |
| `<config-dir>/auth-gateway.token` | `agent auth-gateway serve` (skipped under `--no-auth`) | `0600` in a `0700` parent dir |

`<config-dir>` resolves to `~/.agent/` (respecting `AGENT_CONFIG_DIR`).
