...
Most runtime lookups use `$env` from `@awfixerai/utils` (`packages/utils/src/env.ts`).

`$env` loading order:

1. Existing process environment (`Bun.env`)
2. Project `.env` (`$PWD/.env`) for keys not already set
3. Agent `.env` (`~/.agent/.env`, respecting `AGENT_CONFIG_DIR` / `AGENT_DIR`) for keys not already set
4. Config-root `.env` (`~/.agent/.env`, respecting `AGENT_CONFIG_DIR`) for keys not already set
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE` | Path to the encrypted local broker snapshot cache | Optional in broker mode | Defaults to `~/.agent/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Useful for tests, ephemeral hosts, or relocating the `0600` cache file. |

The gateway has no dedicated env vars — it inherits `OMP_AUTH_BROKER_*`. Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `agent auth-gateway token`.
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.agent/config.yml`; environment variables are fallbacks.
...
| `PI_SUBPROCESS_CMD`          | Overrides subagent spawn command (`agent` / `agent.cmd` resolution bypass)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `OLLAMA_CONTEXT_LENGTH`      | Positive integer context-window override for implicit Ollama discovery; affects agent context budgeting only and does not change Ollama's runtime `num_ctx`                                                                                                                                                                                                                                                                                                                                                                                           |
...
These are consumed via `@awfixerai/utils/dirs` and affect where coding-agent stores data.

| Variable              | Default / behavior                                                            |
| --------------------- | ----------------------------------------------------------------------------- |
| `AGENT_CONFIG_DIR`       | Config root dirname under home (default `.agent`; falls back to `.omp` when migrating) |
| `AGENT_DIR` | Full override for agent directory (default `~/.agent` for the default profile; named profiles use `~/.agent/profiles/<name>/agent`) |
