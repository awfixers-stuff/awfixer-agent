...
```ts
import type { HookAPI } from "@awfixerai/agent/extensibility/hooks";
...
`src/extensibility/hooks/index.ts` and the package subpath `@awfixerai/agent/extensibility/hooks` export:
...
The package root (`@awfixerai/agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
