...
A persistent per-install UUID that identifies a single awfixer-agent installation across sessions. Used as a stable correlation key for server-side dedup of telemetry-style pushes (currently the auto-QA grievance flush from `report_tool_issue`).

## API

Exported from `@awfixerai/utils` (`packages/utils/src/dirs.ts`):
...
- Path: `<base-config-root>/install-id` — i.e. `~/.agent/install-id` by default, respecting `AGENT_CONFIG_DIR`. Resolved against the base config root (`getBaseConfigRoot()`) regardless of the active profile, so every profile on a host shares one install ID (install identity is per-install, not per-profile).
- Format: a single UUID line (trailing `\n`).
- Permissions: file is created with mode `0o600`.
- Lifecycle: independent of `~/.agent/`. Wiping agent state (sessions, settings, DB) does NOT regenerate the install ID; only deleting the `install-id` file itself does.
...
- [environment-variables.md](environment-variables.md) — `AGENT_CONFIG_DIR` controls where `install-id` lives.
