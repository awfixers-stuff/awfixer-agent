...
The compiled macOS `agent` binaries shipped on GitHub Releases are signed with a
...
submission (see [#776](https://github.com/awfixers-stuff/awfixer-agent/issues/776)).
...
- `curl https://agent.awfixer.codes/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/agent-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/agent-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/agent-signing             # upload all five
gh secret list --repo awfixers-stuff/awfixer-agent                       # confirm
```
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/agent-darwin-arm64
```
