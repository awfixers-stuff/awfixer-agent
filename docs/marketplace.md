...
A **plugin** is a directory containing Claude/agent plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Extension modules (`package.json` `omp.extensions` entry points) are not loaded from marketplace installs — they only load for npm-installed or `agent plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.agent/plugins/installed_plugins.json`
...
```
agent plugin marketplace add <source>
agent plugin marketplace remove <name>
agent plugin marketplace update [name]
agent plugin marketplace list
agent plugin discover [marketplace]
agent plugin install [--force] [--scope user|project] name@marketplace
agent plugin uninstall [--scope user|project] name@marketplace
agent plugin upgrade [--scope user|project] [name@marketplace]
agent plugin enable [--scope user|project] name@marketplace
agent plugin disable [--scope user|project] name@marketplace
```
