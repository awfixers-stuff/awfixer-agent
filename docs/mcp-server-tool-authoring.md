...
- Prefer `.omp/mcp.json` (project) or `~/.agent/mcp.json` (user) for explicit control.
...
agent-native MCP config (`.omp/mcp.json`, `~/.agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
