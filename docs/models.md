...
- `src/session/auth-storage.ts` — re-exports `AuthStorage` from `@awfixerai/ai` (`packages/ai/src/auth-storage.ts`); API key + OAuth resolution order
...
- `transport`: `pi-native` only. When set, every model under that provider is sent to an `agent auth-gateway` compatible `baseUrl` via `POST /v1/pi/stream`; `apiKey` is the gateway bearer.
...
providers:
  openai:
    ...
    headers:
      X-Team-Key: "!bw get password agent-team-key"
```
...
1. Load built-in providers/models from `@awfixerai/catalog` (`getBundledProviders` / `getBundledModels`).
...
- `agent models` prints provider-grouped tables of every concrete model, and `agent models canonical` prints the coalesced canonical view
...
`disableStrictTools` is a provider-level flag that applies to all models in the provider. It disables the Anthropic `strict` marker only for tools that agent would otherwise mark strict; it does not change runtime tool argument validation. agent can automatically retry without strict tools after Anthropic reports a strict-grammar-too-large error before the first streamed token, but proxies that reject the `strict` field for other reasons should set this flag explicitly.
...
The built-in vLLM provider can be pointed at a non-default endpoint without declaring a custom discovery type. agent uses vLLM's `/v1/models` metadata and preserves vLLM's `max_model_len` field as the discovered context window.
...
Most model configuration now flows through `models.yml` via `ModelRegistry`. Explicit `.json` / `.jsonc` paths remain supported only when passed programmatically to `ModelRegistry`; the default user config is `~/.agent/models.yml`.
