...
`@awfixerai/natives` is a two-layer package around an ESM loader:
...
There is no current `packages/natives/src` TypeScript wrapper layer. Consumers import functions/classes/enums directly from `@awfixerai/natives`; the type contract is the generated `native/index.d.ts` plus the explicit named exports generated into `native/index.js` by `scripts/gen-enums.ts`.
...
2. Check legacy compiled-binary location:
   - Windows: `%LOCALAPPDATA%/agent` (fallback `%USERPROFILE%/AppData/Local/agent`)
   - non-Windows: `~/.local/bin`
3. Fall back to packaged `native/` and executable directory candidates.

`getNativesDir()` uses `$XDG_DATA_HOME/agent/natives` when `$XDG_DATA_HOME/agent` exists; otherwise it uses `~/.agent/natives`.
...
1. Consumer imports from `@awfixerai/natives`.
...
- **Platform leaf package**: Per-platform npm package `@awfixer-agent/agent-natives-<tag>` that carries one platform's prebuilt `.node`. The core depends on every leaf via `optionalDependencies`; the package manager installs only the host-matching one (`os`/`cpu`).
