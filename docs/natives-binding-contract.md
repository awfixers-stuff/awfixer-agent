...
This document defines the JS/TS contract between `@awfixerai/natives` callers and the loaded N-API addon.
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@awfixerai/natives`.
