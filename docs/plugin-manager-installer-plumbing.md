...
This document describes how `agent plugin` npm/git/link operations mutate plugin state on disk and how installed npm/git/link plugins become runtime capabilities (tools and extensions today, hooks/commands path resolution available). Marketplace installs use separate marketplace registries and cache plumbing; see `docs/marketplace.md`.
...
```text
agent plugin <npm/link action> ...
  -> src/commands/plugin.ts
  -> runPluginCommand(...) in src/cli/plugin-cli.ts
  -> PluginManager method (install/list/uninstall/link/...)
  -> mutate ~/.agent/plugins/{package.json,node_modules,agent-plugins.lock.json}
...
agent plugin install name@marketplace / agent install name@marketplace
  -> MarketplaceManager
  -> mutate ~/.agent/marketplaces.json, ~/.agent/plugins/installed_plugins.json, cache dirs
...
Global plugin state lives under `~/.agent/plugins`:
...
- `agent-plugins.lock.json` — runtime state for npm/link plugins:
...
- `~/.agent/marketplaces.json` — configured marketplace catalogs
- `~/.agent/plugins/installed_plugins.json` — user-scoped marketplace installs
- `<cwd>/.omp/plugins/installed_plugins.json` — project-scoped marketplace installs when available
- `~/.agent/plugins/cache/{marketplaces,plugins}/` — cached catalogs and plugin directories
...
- A package missing `agent`/`pi` is still installable and listable.
- Runtime plugin loading (`getEnabledPlugins`) skips packages without `agent`/`pi` manifest.
...
4. Run `bun install <packageSpec>` in `~/.agent/plugins`.
...
- `agent plugin install pkg@newVersion` updates dependency and lockfile version.
...
1. Read plugin dependency map from `~/.agent/plugins/package.json`.
...
`link` supports local plugin development by symlinking a local package into `~/.agent/plugins/node_modules/<pkg.name>`.
...
- skip if manifest (`agent`/`pi`) absent
