...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@awfixerai/agent`
  - `@mariozechner/pi-agent-core` → `@awfixerai/agent-core`
  - `@mariozechner/pi-tui` → `@awfixerai/tui`
  - `@mariozechner/pi-ai` → `@awfixerai/ai`
  - `@mariozechner/pi-utils` → `@awfixerai/utils`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/pi-coding-agent` → `@awfixerai/agent`, and so on).
- The bare `typebox` package is not an `@awfixer-agent/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
- In `packages/coding-agent`, use `logger` from `@awfixerai/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@awfixerai/natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@awfixerai/natives` | Native implementation with a small TS wrapper |
...
| `formatSize` from `pi-coding-agent`                              | `formatBytes` from `@awfixerai/utils`                                                            |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@awfixerai/natives`
