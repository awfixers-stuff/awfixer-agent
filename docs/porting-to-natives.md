...
`@awfixerai/natives` no longer has a `packages/natives/src/<module>` TypeScript wrapper layer. The package root points at generated native artifacts:
...
Consumers import directly from `@awfixerai/natives`. The generated declarations and explicit ESM exports are produced during `bun --cwd=packages/natives run build`.
...
- Import the new export directly from `@awfixerai/natives`.
...
For compiled binaries or Windows staging, delete the versioned addon cache shown in the loader error (normally under `~/.agent/natives/<version>` unless `$XDG_DATA_HOME/agent` is used).
