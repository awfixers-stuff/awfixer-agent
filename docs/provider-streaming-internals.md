...
This document explains how token/tool streaming is normalized in `@awfixerai/ai`, then propagated through `@awfixerai/agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@awfixerai/ai`
