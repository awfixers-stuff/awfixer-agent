...
The SDK is the in-process integration surface for `@awfixerai/agent`.
...
```bash
bun add @awfixerai/agent
```

## Entry points

`@awfixerai/agent` exports the SDK APIs from the package root (and also via `@awfixerai/agent/sdk`).
...
```ts
import { createAgentSession } from "@awfixerai/agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@awfixerai/agent";
...
```ts
import { SessionManager } from "@awfixerai/agent";
