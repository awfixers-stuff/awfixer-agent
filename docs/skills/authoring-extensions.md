...
description: Use when creating a new agent extension. Covers ExtensionAPI, factory signature, tool/command/event registration, and local-dev testing.
---

# Authoring Extensions

Extensions are the primary way to add capabilities to `awfixer-agent`. A single extension module can register tools the LLM can call, slash commands users can invoke, and event handlers that run throughout the session lifecycle — all from one TypeScript file.
...
That is a working extension. Drop it into `~/.agent/extensions/hello.ts` and restart agent to see the notification.
...
agent loads extension modules from these sources:

1. Native `.omp` locations discovered through the capability system:
   - `<cwd>/.omp/extensions/`
   - `~/.agent/extensions/`
   - legacy extension paths listed in `.omp/settings.json#extensions` or `~/.agent/settings.json#extensions`
2. Installed plugins under `~/.agent/plugins/node_modules` (`agent plugin install` npm/git specs, or `agent plugin link`) via their `omp.extensions`/`pi.extensions` manifests. Marketplace cache installs do not feed extension modules — they surface skills/commands/hooks/tools/MCP only.
3. Explicit configured paths passed by the CLI (`agent --extension ./my-ext.ts`, also `-e`; `--hook` is treated as an alias) and by the `extensions:` setting in config.

The runtime de-duplicates by resolved absolute path — first seen wins.

When a path points to a directory, agent resolves the entry point in this order:

1. `package.json` with `omp.extensions` (or legacy `pi.extensions`) field
2. `index.ts`
3. `index.js`

When scanning an `extensions/` directory, agent also loads direct `*.ts`/`*.js` files and one-level subdirectories that have `index.ts`, `index.js`, or a manifest.
...
To package an extension as an installable plugin, add an `agent` field to `package.json`:
...
| Legacy hook module already exists | **Hook** (`HookAPI` from `@awfixerai/agent/extensibility/hooks`) |
...
agent writes structured logs to a rotating file under `~/.agent/logs/` (debug level is always on; nothing is written to the console, which would corrupt the TUI). Tail today's log to see extension load diagnostics:

```
tail -f ~/.agent/logs/agent.$(date +%F).log
```
