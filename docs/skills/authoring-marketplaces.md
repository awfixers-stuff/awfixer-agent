...
The catalog file lives at either `.omp-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. agent prefers the `.omp-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
```json
{
  ...
  "metadata": {
    "description": "Official Acme plugins for awfixer-agent"
  },
  "plugins": [
    {
      "name": "acme-linter",
...
> Note: extension modules declared via `package.json` `omp.extensions` are **not** loaded from marketplace installs — that mechanism only applies to npm-installed or `agent plugin link`ed plugins. Ship marketplace plugin behavior through the conventional directories above.
...
```
agent plugin marketplace add owner/repo
agent plugin install name@marketplace-name
```

Scope behavior:

- **user** (default) — installed in `~/.agent/plugins/installed_plugins.json`, available in all projects
...
1. Create `marketplace.json` at `.omp-plugin/marketplace.json` (agent-only) or `.claude-plugin/marketplace.json` (shared with Claude Code) in a new Git repo.
