...
```
cp -r . ~/.agent/extensions/hello-extension
```

Restart `agent`. You will see the startup notification immediately.
...
```
agent --extension ./hello-extension
```

## Usage

After loading, type `/hello` or `/hello Ada` in the agent prompt. The command sends a visible greeting custom message into the conversation and shows a "Message sent!" notification.
