...
- syntax highlighting colors used by native highlighter (`@awfixerai/natives`)
...
- overridden by `AGENT_DIR` (`$AGENT_DIR/themes`)
...
- default agent dir: `~/.agent`
- effective default file: `~/.agent/config.yml`
...
1. Create file in custom themes dir, e.g. `~/.agent/themes/my-theme.json`.
