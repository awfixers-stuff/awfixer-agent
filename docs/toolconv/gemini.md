...
> **agent dialect note:** because this convention carries no native in-band reasoning marker, the agent `gemini` dialect layers a sibling fenced ` ```thinking ` block (closed by a bare ` ``` `, exactly like ` ```tool_code `) so prompt-driven Gemini / Gemma-3 deployments can express reasoning in-band. This is an agent convention, not part of Google's format.
