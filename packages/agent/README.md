...
Stateful agent with tool execution and event streaming. Built on `@awfixerai/ai`.

## Installation

```bash
npm install @awfixerai/agent-core
```
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@awfixerai/ai`).
...
```typescript
import { Agent, streamProxy } from "@awfixerai/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@awfixerai/agent-core";
