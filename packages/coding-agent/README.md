...
Core implementation package for the `agent` coding agent in the `awfixer-agent` monorepo.

For installation, setup, provider configuration, model roles, slash commands, and full CLI reference, see:
- [Monorepo README (local)](../../README.md)
- [Monorepo README (GitHub)](https://github.com/awfixers-stuff/awfixer-agent#readme)
...
The agent supports three mutually-exclusive memory backends, selected via the `memory.backend` setting (Settings → Memory tab, or `~/.agent/config.yml`):
