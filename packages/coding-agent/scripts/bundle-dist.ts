...
// (native addon, mupdf).
const ALWAYS_EXTERNAL = ["mupdf", "@awfixerai/natives", "@huggingface/transformers", "fastembed", "onnxruntime-node"];
...
async function cleanBundleOutputs(): Promise<void> {
	// dist/ is shared with the dev binary (dist/agent); only remove this
