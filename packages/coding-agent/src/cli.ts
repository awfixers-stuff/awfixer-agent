...
import type { CliConfig } from "@awfixerai/utils/cli";
import {
	APP_NAME,
	getActiveProfile,
	MIN_BUN_VERSION,
...
import { declareWorkerHostEntry, installWorkerInbox } from "@awfixerai/utils/worker-host";
...
// `@awfixerai/utils/env` eagerly loads `.env` from the agent directory at
// import time, so it must not be imported before `setProfile` runs.

async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@awfixerai/utils/cli");
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker, startServer } = await import("@awfixerai/stats");
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@awfixerai/stats/sync-worker");
...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			setProfile(resolveProfileEnv(process.env.AGENT_PROFILE, process.env.PI_PROFILE));
		}
	...
	// `@awfixerai/utils/env` here would snapshot the wrong agent `.env`.
