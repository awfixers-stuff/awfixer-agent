...
import { truncateToWidth } from "@awfixerai/tui/utils";
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@awfixerai/utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@awfixerai/stats");
	const stats = await getDashboardStats();
