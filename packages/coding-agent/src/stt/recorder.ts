...
import { $which, logger, Snowflake } from "@awfixerai/utils";
...
export async function startRecording(outputPath: string): Promise<RecordingHandle> {
	...
	if (recorders.length === 0) {
		throw new Error("No audio recorder available — run `agent setup speech`");
	}
	...
	throw new Error(`No audio recorder could start — run \`agent setup speech\`.\n${failures.join("\n")}`);
}
