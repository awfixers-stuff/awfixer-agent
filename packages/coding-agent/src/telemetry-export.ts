 ...
 *
 * oh-my-pi's agent core (`@awfixerai/agent-core`) emits OpenTelemetry GenAI
 ...
 */
import { logger, postmortem } from "@awfixerai/utils";
...
async function registerProvider(): Promise<void> {
	...
	const tracerProvider = new NodeTracerProvider({
		resource: resourceFromAttributes({
			"service.name": process.env.OTEL_SERVICE_NAME ?? "awfixer-agent",
		}),
