...
import { clearCache, readFile } from "@awfixerai/agent/capability/fs";
import type { Rule } from "@awfixerai/agent/capability/rule";
import type { LoadContext } from "@awfixerai/agent/capability/types";
import { getProjectPathCandidates } from "@awfixerai/agent/discovery/agents";
import {
...
describe("agents provider project-level discovery", () => {
	...
	describe("skills", () => {
		...
		test("project walk-up skips home directory (no repo root)", async () => {
			// Regression for https://github.com/awfixers-stuff/awfixer-agent/issues/1116:
