...
// compiled-binary mode now routes through `agent-legacy-bundled:` virtual
...
describe("legacy pi compat package-root override validation (issue #2168)", () => {
	it("keeps overrides whose filesystem targets exist", () => {
		const candidates = {
			"@awfixerai/ai": "/tmp/exists-ai.js",
			"@awfixerai/utils": "/tmp/exists-utils.js",
		};
	...
	it("drops overrides whose filesystem targets are missing on disk", () => {
		const candidates = {
			"@awfixerai/ai": "/tmp/exists-ai.js",
			"@awfixerai/agent": "/tmp/exists-shim.js",
			"@awfixerai/utils": "/$bunfs/root/packages/utils/src/index.js",
			"@awfixerai/tui": "/$bunfs/root/packages/tui/src/index.js",
		};
		...
		expect(result).not.toHaveProperty("@awfixerai/utils");
		expect(result).not.toHaveProperty("@awfixerai/tui");
	});
	...
	it("keeps virtual agent-legacy-bundled: entries without touching the filesystem (issue #3423)", () => {
		...
		let probed = false;
		const candidates = {
			"@awfixerai/ai": "agent-legacy-bundled:@awfixerai/ai",
			"@awfixerai/agent": "agent-legacy-bundled:@awfixerai/agent",
			"@awfixerai/agent-core": "agent-legacy-bundled:@awfixerai/agent-core",
			"@awfixerai/natives": "agent-legacy-bundled:@awfixerai/natives",
			"@awfixerai/tui": "agent-legacy-bundled:@awfixerai/tui",
			"@awfixerai/utils": "agent-legacy-bundled:@awfixerai/utils",
		};
	...
	it("mixes virtual and filesystem entries: virtuals always pass, filesystems gated", () => {
		const candidates = {
			"@awfixerai/ai": "agent-legacy-bundled:@awfixerai/ai",
			"@awfixerai/agent": "/dev/source/legacy-pi-coding-agent-shim.ts",
			"@awfixerai/tui": "/missing/path.ts",
		};
...
describe("legacy pi compat typebox shim path resolution (issues #3414, #3423)", () => {
	it("returns the virtual specifier in compiled-binary mode without touching the filesystem", () => {
		...
		expect(result).toBe("agent-legacy-bundled:typebox");
		expect(probed).toBe(false);
	});
