/**
 * Regression for https://github.com/awfixers-stuff/awfixer-agent/issues/3031
 *
...
} from "@awfixerai/agent/mnemopi/embed-client";
import type { MnemopiEmbedWorkerInbound, MnemopiEmbedWorkerOutbound } from "@awfixerai/agent/mnemopi/embed-protocol";
...
describe("issue #3031 — mnemopi embeddings live in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		const script =
			'const { smokeTestMnemopiEmbedWorker } = await import("@awfixerai/agent/mnemopi/embed-client"); await smokeTestMnemopiEmbedWorker({ timeoutMs: 15000 });';
