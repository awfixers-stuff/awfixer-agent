...
import { listOmpExtensionRoots } from "@awfixerai/agent/discovery/omp-extension-roots";
import { getEnabledPlugins } from "@awfixerai/agent/extensibility/plugins/loader";
import { PluginManager } from "@awfixerai/agent/extensibility/plugins/manager";
import { MarketplaceManager, readInstalledPluginsRegistry } from "@awfixerai/agent/extensibility/plugins/marketplace";
...
function mockPluginManagerPaths(root: string) {
	return [
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "agent-plugins.lock.json")),
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "agent-plugins.lock.json")).json();
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		...
		try {
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "agent-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "agent-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "agent-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "agent-plugins.lock.json")).json();
