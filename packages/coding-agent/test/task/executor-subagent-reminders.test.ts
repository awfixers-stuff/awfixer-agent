...
import type { ExtensionActions, LoadExtensionsResult } from "@awfixerai/agent/extensibility/extensions/types";
...
import type { AuthStorage } from "@awfixerai/agent/session/auth-storage";
import {
	finalizeSubprocessOutput,
	runSubprocess,
	SUBAGENT_WARNING_MISSING_YIELD,
} from "@awfixerai/agent/task/executor";
import type { AgentDefinition } from "@awfixerai/agent/task/types";
import { EventBus } from "@awfixerai/agent/utils/event-bus";
import { AgentBusyError, type AgentTelemetryConfig, type Tracer } from "@awfixerai/agent-core";
import { type AssistantMessage, Effort } from "@awfixerai/ai";
...
describe("runSubprocess yield reminders", () => {
	...
	const baseOptions = {
		...
		modelRegistry: {
			refresh: async () => {},
		} as unknown as import("@awfixerai/agent/config/model-registry").ModelRegistry,
		enableLsp: false,
	};
