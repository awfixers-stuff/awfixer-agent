...
import type { CustomToolContext } from "@awfixerai/agent/extensibility/custom-tools";
import type { ReadonlySessionManager } from "@awfixerai/agent/session/session-manager";
import {
	getImageGenTools,
	getImageGenToolsWithRegistry,
	imageGenTool,
	setPreferredImageProvider,
} from "@awfixerai/agent/tools/image-gen";
...
describe("imageGenTool", () => {
	...
	it("routes xAI image generation with xAI-only aspect ratios", async () => {
		...
		expect(captured.userAgent).toBe("awfixer-agent/xai");
		expect(requestBody).toMatchObject({
