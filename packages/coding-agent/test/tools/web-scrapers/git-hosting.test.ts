...
import { handleGitHub, parseGitHubUrl, stripActionsLogTimestamps } from "@awfixerai/agent/web/scrapers/github";
import { handleGitHubGist } from "@awfixerai/agent/web/scrapers/github-gist";
...
describe("parseGitHubUrl — Actions", () => {
	it("classifies a workflow run URL", () => {
		const gh = parseGitHubUrl("https://github.com/awfixers-stuff/awfixer-agent/actions/runs/27070071296");
	...
	it("classifies a job URL using the web-form singular `job` segment", () => {
		const gh = parseGitHubUrl(
			"https://github.com/awfixers-stuff/awfixer-agent/actions/runs/27070071296/job/79897931171",
		);
		expect(gh).toEqual({
			type: "actions-job",
			owner: "can1357",
	...
	it("classifies a job URL using the API-form plural `jobs` segment", () => {
		const gh = parseGitHubUrl(
			"https://github.com/awfixers-stuff/awfixer-agent/actions/runs/27070071296/jobs/79897931171",
		);
	...
	it("does not treat non-run Actions URLs (e.g. workflow files) as runs/jobs", () => {
		expect(parseGitHubUrl("https://github.com/awfixers-stuff/awfixer-agent/actions/workflows/ci.yml")?.type).toBe(
			"other",
		);
		expect(parseGitHubUrl("https://github.com/awfixers-stuff/awfixer-agent/actions")?.type).toBe("other");
	});

	it("does not misparse a run URL with a non-numeric id", () => {
		expect(parseGitHubUrl("https://github.com/awfixers-stuff/awfixer-agent/actions/runs/latest")?.type).toBe("other");
	});
...
describe("parseGitHubUrl — commit", () => {
	it("classifies a commit URL with a full SHA", () => {
		const gh = parseGitHubUrl(
			"https://github.com/awfixers-stuff/awfixer-agent/commit/c1a1cb6149e73b345919dd4cf629b0d9ac74fb57",
		);
		expect(gh).toEqual({
			type: "commit",
			owner: "can1357",
	...
	it("accepts an abbreviated SHA", () => {
		expect(parseGitHubUrl("https://github.com/awfixers-stuff/awfixer-agent/commit/c1a1cb6")).toEqual({
			type: "commit",
			owner: "can1357",
			repo: "oh-my-pi",
			ref: "c1a1cb6",
	...
	it("falls back to `other` for a bare /commit segment with no SHA", () => {
		expect(parseGitHubUrl("https://github.com/awfixers-stuff/awfixer-agent/commit")?.type).toBe("other");
	});
});
