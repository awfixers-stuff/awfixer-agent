...
- **Session log parsing**: Reads JSONL session logs from `~/.agent/sessions/`
...
```typescript
import { getDashboardStats, syncAllSessions } from "@awfixerai/stats";
...
- **Session logs**: `~/.agent/sessions/` (JSONL files)
- **Stats database**: `~/.agent/stats.db` (SQLite)
