...
`autoawfixer` is a self-hosted GitHub triage-and-fix bot that drives [`agent --mode rpc`](https://github.com/awfixers-stuff/awfixer-agent) as a subprocess. On every issue opened in an allowlisted repository it classifies the issue, applies labels, then branches into one of: reproduce → fix → PR (`bug` / `documentation`), single-comment answer (`question`), single thoughtful comment (`enhancement` / `proposal`), or brief comment (`invalid` / `duplicate`). Follow-up comments and PR review comments resume the same agent session so the agent keeps its prior reasoning. If the orchestrator restarts mid-task, the dispatcher resumes the same session via `agent --continue` from the per-issue `session_dir`, so an interrupted task re-enters its prior reasoning instead of restarting from scratch. The orchestrator runs as a single FastAPI process inside Docker with SQLite-backed durable event state.

## Architecture & Data Flow

Webhook → durable queue → async dispatcher → per-issue git worktree → agent RPC subprocess + host tools.
...
4. `queue.WorkerPool._dispatch_loop` atomically claims `state='queued'` rows under `BEGIN IMMEDIATE`, guarded by an in-process `_inflight` set keyed by `(owner, repo, number)` to serialize per-issue work. Cap: `AUTOAWFIXER_MAX_CONCURRENCY` (default 8).
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `agent --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `AUTOAWFIXER_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
7. Inside the subprocess the agent uses **built-in agent tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
Task runner is `bun` against the **monorepo root** `package.json`. autoawfixer itself no longer ships a `package.json`; every recipe lives at the root under the `autoawfixer:*` namespace. Local venv (no docker): `bun run autoawfixer:install` runs `pip install -e 'python/autoawfixer[dev]'`. From there:

```
bun run test:py                   # pytest -x python/omp-rpc/tests python/autoawfixer/tests
bun run autoawfixer:test:integration   # AUTOAWFIXER_INTEGRATION=1, requires agent on PATH
bun run autoawfixer:serve              # python -m autoawfixer serve on the host
```
...
bun run autoawfixer:reset              # `down -v` + drop the pi image
```

Frontend (Vite + SolidJS, in `web/` — still a bun workspace):

```
bun run autoawfixer:web:dev            # vite dev server with proxy to :8080
bun run autoawfixer:web:build          # produce src/static/ bundle
bun --cwd=python/autoawfixer/web run typecheck   # tsc --noEmit
```

In-container CLI (`autoawfixer` console script → `autoawfixer.cli:main`): no root aliases — invoke directly:

```
docker compose --project-directory python/autoawfixer exec autoawfixer autoawfixer triage owner/repo#N
docker compose --project-directory python/autoawfixer exec autoawfixer autoawfixer replay <delivery_id>
docker compose --project-directory python/autoawfixer exec autoawfixer autoawfixer status
docker compose --project-directory python/autoawfixer exec autoawfixer autoawfixer cleanup owner/repo#N
```

HTTP / sqlite / webhook inspection is unaliased — use `curl http://localhost:${AUTOAWFIXER_BIND_PORT:-8080}/{healthz,readyz,events,issues}` and `docker compose --project-directory python/autoawfixer exec autoawfixer sqlite3 /data/autoawfixer.sqlite` directly.

Lint + format: TypeScript via Biome (config in `biome.json`), Python via Ruff (config in `pyproject.toml`). Root recipes cover both languages — `bun run lint` / `bun run fix` apply to the whole monorepo including autoawfixer. `bun run lint:py` / `bun run fix:py` scope to Python only.
...
- **Config**: `pydantic-settings` `Settings` in `config.py` with `AUTOAWFIXER_*` env prefix (e.g. `AUTOAWFIXER_MAX_CONCURRENCY`, `AUTOAWFIXER_REPO_ALLOWLIST`). Access only via `get_settings()` (`@cache` singleton). Tests must call `reset_settings_cache()` after mutating env.
...
- `src/worker.py` — synchronous agent RPC driver, prompt assembly via `persona`.
...
- **Fixtures** (`tests/conftest.py`):
  - `env` — `monkeypatch`-sets all required `AUTOAWFIXER_*` env vars and calls `reset_settings_cache()` before/after.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `AUTOAWFIXER_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `agent` on `PATH`. Don't enable it in default `bun run test:py`.
