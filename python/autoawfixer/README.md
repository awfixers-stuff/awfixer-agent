...
Self-hosted GitHub triage bot. Drives [`agent --mode rpc`](https://github.com/awfixers-stuff/awfixer-agent)
...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `AUTOAWFIXER_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same agent session
...
- **autoawfixer** — FastAPI + sqlite event queue + `WorkerPool` running `agent` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from autoawfixer, executes REST + `git push`.
...
→ `worker.run_task` spawns `agent --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `AUTOAWFIXER_MODEL` (CSV).

The agent uses agent's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host agent doesn't route through the gateway). autoawfixer lives inside the
awfixer-agent monorepo at `python/autoawfixer/`; both the docker build context and the
`/work/pi` bind mount default to the parent monorepo (`../..`). Override
`AGENT_ROOT` only if you want a different awfixer-agent checkout backing the build
and runtime.

Bot account needs **Write** on every repo in `AUTOAWFIXER_REPO_ALLOWLIST`. A
...
cp .env.example .env
$EDITOR .env
openssl rand -hex 32              # AUTOAWFIXER_GH_PROXY_HMAC_KEY
...
comment out `AUTOAWFIXER_GH_PROXY_URL` / `AUTOAWFIXER_GH_PROXY_HMAC_KEY` and set
...
autoawfixer does not ship a tunnel. Cloudflare, smee, ngrok are all fine. The
...
The container entrypoint is `python -m autoawfixer serve`. Other commands run
inside the running container:

```bash
docker compose exec autoawfixer autoawfixer triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec autoawfixer autoawfixer replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec autoawfixer autoawfixer status                   # dump issues table
docker compose exec autoawfixer autoawfixer cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run autoawfixer:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`autoawfixer:dev`, `autoawfixer:build`, `autoawfixer:up`, `autoawfixer:down`,
`autoawfixer:logs`, `autoawfixer:restart`, `autoawfixer:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
AUTOAWFIXER_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `agent --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `agent` on
...
- gh-proxy has no host port. The `autoawfixer_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `AUTOAWFIXER_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `AUTOAWFIXER_GIT_AUTHOR_NAME` +
  `AUTOAWFIXER_GIT_AUTHOR_EMAIL`.
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `AUTOAWFIXER_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `AUTOAWFIXER_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/autoawfixer.log.jsonl`.
...
| Container exits with `AGENT_ROOT … missing` | `/work/pi` mount empty inside the container; on the host either run `docker compose` from `python/autoawfixer/` so `AGENT_ROOT` defaults to `../..`, or export `AGENT_ROOT` to a valid awfixer-agent checkout. |
| `git push: Authentication required` | Bot PAT lacks push, or `AUTOAWFIXER_BOT_LOGIN` does not identify the PAT account's mention handle (production: `roboomp`, no `@`/`[bot]`). |
| `refusing to push: commit author identity mismatch` | Some commit not authored as `AUTOAWFIXER_GIT_AUTHOR_*`. The error lists the offending shas; `git commit --amend --reset-author --no-edit`. |
...
| `Failed to load pi_natives` | Wrong arch / missing native. `bun run agent:image` then `bun run autoawfixer:build`. |
| `No API key found for <provider>` | `~/.agent/models.container.yml` mount missing or provider id mismatch with `AUTOAWFIXER_MODEL`. |
...
src/
  ...
  worker.py          synchronous agent RPC driver, prompt assembly, env scrubbing
...
tests/               pytest unit suite + one AUTOAWFIXER_INTEGRATION=1 smoke test
