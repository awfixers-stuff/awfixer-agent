...
if [ ! -d "$AGENT_ROOT/packages/coding-agent" ]; then
    echo "autoawfixer: AGENT_ROOT=$AGENT_ROOT does not look like a pi checkout (no packages/coding-agent/)" >&2
    exit 1
fi
...
touch /data/autoawfixer.sqlite
chown root:root /data/autoawfixer.sqlite
chmod 0600 /data/autoawfixer.sqlite
for db_file in /data/autoawfixer.sqlite-wal /data/autoawfixer.sqlite-shm; do
    if [ -e "$db_file" ]; then
