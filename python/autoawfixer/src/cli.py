...
from autoawfixer.db import INACTIVE_EVENT_STATES, get_database
from autoawfixer.logging_config import configure_logging
...
from autoawfixer.proxy_client import GitHubProxyClient
...
def triage(issue_ref: str, wait_timeout: float | None) -> None:
    ...
    if not cfg.allows(repo_full):
        click.echo(f"refusing: {repo_full} not in AUTOAWFIXER_REPO_ALLOWLIST", err=True)
        sys.exit(2)
