"""Env-driven configuration for autoawfixer."""
...
class Settings(BaseSettings):
    ...
    bot_login: str = Field(..., alias="AUTOAWFIXER_BOT_LOGIN")
    git_author_name: str | None = Field(None, alias="AUTOAWFIXER_GIT_AUTHOR_NAME")
    git_author_email: str = Field(..., alias="AUTOAWFIXER_GIT_AUTHOR_EMAIL")
    repo_allowlist_raw: str = Field("", alias="AUTOAWFIXER_REPO_ALLOWLIST")
    pr_review_enabled: bool = Field(True, alias="AUTOAWFIXER_PR_REVIEW_ENABLED")
    ...
    # vouch GitHub Action labels the PR `vouch_review_label`, so autoawfixer reviews
    ...
    pr_review_trigger: Literal["open", "vouched_label"] = Field("open", alias="AUTOAWFIXER_PR_REVIEW_TRIGGER")
    vouch_review_label: str = Field("vouched", alias="AUTOAWFIXER_VOUCH_REVIEW_LABEL")
    ...
    vouch_review_labeler: str = Field("github-actions[bot]", alias="AUTOAWFIXER_VOUCH_REVIEW_LABELER")
    ...
    gh_proxy_url: str | None = Field(None, alias="AUTOAWFIXER_GH_PROXY_URL")
    gh_proxy_hmac_key: SecretStr | None = Field(None, alias="AUTOAWFIXER_GH_PROXY_HMAC_KEY")
    # Bind address for `python -m autoawfixer.proxy serve`. Internal-only by
    # default; gh-proxy never exposes a host port.
    gh_proxy_bind_host: str = Field("0.0.0.0", alias="AUTOAWFIXER_GH_PROXY_BIND_HOST")
    gh_proxy_bind_port: int = Field(8081, alias="AUTOAWFIXER_GH_PROXY_BIND_PORT")
    ...
    gh_proxy_max_body_bytes: int = Field(1 << 20, alias="AUTOAWFIXER_GH_PROXY_MAX_BODY_BYTES")
    ...
    gh_proxy_git_timeout_seconds: float = Field(60.0, alias="AUTOAWFIXER_GH_PROXY_GIT_TIMEOUT_SECONDS")

    # Model selection
    model: str = Field("anthropic/claude-sonnet-4-6", alias="AUTOAWFIXER_MODEL")
    provider: str | None = Field(None, alias="AUTOAWFIXER_PROVIDER")
    thinking_level: ThinkingLevel = Field("high", alias="AUTOAWFIXER_THINKING")

    # Runtime
    max_concurrency: int = Field(8, alias="AUTOAWFIXER_MAX_CONCURRENCY")
    task_timeout_seconds: float = Field(2400.0, alias="AUTOAWFIXER_TASK_TIMEOUT_SECONDS")
    task_timeout_hard_grace_seconds: float = Field(60.0, alias="AUTOAWFIXER_TASK_TIMEOUT_HARD_GRACE_SECONDS")
    request_timeout_seconds: float = Field(120.0, alias="AUTOAWFIXER_REQUEST_TIMEOUT_SECONDS")
    ...
    event_max_retries: int = Field(3, alias="AUTOAWFIXER_EVENT_MAX_RETRIES")
    event_retry_delays_raw: str = Field("30,120,600", alias="AUTOAWFIXER_EVENT_RETRY_DELAYS_SECONDS")
    ...
    # Set to 0 to disable.
    task_completion_max_reminders: int = Field(2, alias="AUTOAWFIXER_TASK_COMPLETION_MAX_REMINDERS")
    omp_command: str = Field("omp", alias="AUTOAWFIXER_OMP_COMMAND")
    ...
    shutdown_drain_timeout_seconds: float = Field(25.0, alias="AUTOAWFIXER_SHUTDOWN_DRAIN_TIMEOUT_SECONDS")
    shutdown_kill_timeout_seconds: float = Field(5.0, alias="AUTOAWFIXER_SHUTDOWN_KILL_TIMEOUT_SECONDS")

    # Paths
    workspace_root: Path = Field(Path("./data/workspaces"), alias="AUTOAWFIXER_WORKSPACE_ROOT")
    sqlite_path: Path = Field(Path("./data/autoawfixer.sqlite"), alias="AUTOAWFIXER_SQLITE_PATH")
    log_dir: Path = Field(Path("./data/logs"), alias="AUTOAWFIXER_LOG_DIR")

    # Server
    bind_host: str = Field("0.0.0.0", alias="AUTOAWFIXER_BIND_HOST")
    bind_port: int = Field(8080, alias="AUTOAWFIXER_BIND_PORT")

    # Dev-only replay header value; if empty, /replay is disabled
    replay_token: SecretStr | None = Field(None, alias="AUTOAWFIXER_REPLAY_TOKEN")
    ...
    rate_limit_window_seconds: float = Field(3600.0, alias="AUTOAWFIXER_RATE_LIMIT_WINDOW_SECONDS")
    rate_limit_default: int = Field(3, alias="AUTOAWFIXER_RATE_LIMIT_DEFAULT")
    rate_limit_contributor: int = Field(10, alias="AUTOAWFIXER_RATE_LIMIT_CONTRIBUTOR")
    rate_limit_unlimited_raw: str = Field("", alias="AUTOAWFIXER_RATE_LIMIT_UNLIMITED")
    ...
    maintainer_logins_raw: str = Field("", alias="AUTOAWFIXER_MAINTAINER_LOGINS")
    ...
    reviewer_bots_raw: str = Field("", alias="AUTOAWFIXER_REVIEWER_BOTS")
    ...
    question_autoclose_enabled: bool = Field(True, alias="AUTOAWFIXER_QUESTION_AUTOCLOSE_ENABLED")
    question_autoclose_hours: float = Field(4.0, alias="AUTOAWFIXER_QUESTION_AUTOCLOSE_HOURS")
    question_autoclose_scan_seconds: float = Field(60.0, alias="AUTOAWFIXER_QUESTION_AUTOCLOSE_SCAN_SECONDS")
    ...
    natives_cache_enabled: bool = Field(True, alias="AUTOAWFIXER_NATIVES_CACHE_ENABLED")
    natives_cache_root: Path = Field(Path("/data/cache/pi-natives"), alias="AUTOAWFIXER_NATIVES_CACHE_ROOT")
    natives_cache_max_entries_per_repo: int = Field(8, alias="AUTOAWFIXER_NATIVES_CACHE_MAX_ENTRIES_PER_REPO")
    natives_cache_max_bytes: int = Field(4 * 1024**3, alias="AUTOAWFIXER_NATIVES_CACHE_MAX_BYTES")
    natives_cache_gc_interval_seconds: float = Field(3600.0, alias="AUTOAWFIXER_NATIVES_CACHE_GC_INTERVAL_SECONDS")
    ...
    def _require_bot_login(cls, value: str) -> str:
        ...
        if cleaned.endswith("[bot]"):
            cleaned = cleaned[:-5]
        if not cleaned:
            raise ValueError("AUTOAWFIXER_BOT_LOGIN must be a non-empty GitHub login")
        return cleaned
    ...
    def _blank_replay_disables(cls, value: object) -> object:
        ...
        # AUTOAWFIXER_REPLAY_TOKEN becomes SecretStr("") which the server would
        # happily compare against an empty X-Autoawfixer-Replay-Token header.
    ...
    def _validate_proxy_or_pat(self) -> Settings:
        ...
        if has_token and has_url:
            raise ValueError(
                "GITHUB_TOKEN and AUTOAWFIXER_GH_PROXY_URL are mutually exclusive — "
                "set ONE to choose between direct-PAT and gh-proxy modes."
            )
        if has_url != has_key:
            raise ValueError(
                "AUTOAWFIXER_GH_PROXY_URL and AUTOAWFIXER_GH_PROXY_HMAC_KEY must both be set together (or both empty)."
            )
        if not has_token and not has_url:
            raise ValueError(
                "no GitHub access configured: set GITHUB_TOKEN, or set "
                "AUTOAWFIXER_GH_PROXY_URL + AUTOAWFIXER_GH_PROXY_HMAC_KEY to use gh-proxy."
            )
        return self
    ...
    def model_pool(self) -> tuple[str, ...]:
        """AUTOAWFIXER_MODEL may be a single id or a comma-separated list; this
    ...
    def resolved_author_name(self) -> str:
        """Falls back to bot_login if AUTOAWFIXER_GIT_AUTHOR_NAME isn't set."""
...
class _ProxyEnvLoader(BaseSettings):
    """Minimal env loader for `python -m autoawfixer.proxy serve`.
    ...
    gh_proxy_hmac_key: SecretStr = Field(..., alias="AUTOAWFIXER_GH_PROXY_HMAC_KEY")
    gh_proxy_bind_host: str = Field("0.0.0.0", alias="AUTOAWFIXER_GH_PROXY_BIND_HOST")
    gh_proxy_bind_port: int = Field(8081, alias="AUTOAWFIXER_GH_PROXY_BIND_PORT")
    workspace_root: Path = Field(Path("./data/workspaces"), alias="AUTOAWFIXER_WORKSPACE_ROOT")
    log_dir: Path = Field(Path("./data/logs"), alias="AUTOAWFIXER_LOG_DIR")
