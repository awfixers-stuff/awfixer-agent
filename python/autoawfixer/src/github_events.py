...
from autoawfixer.db import issue_key
from autoawfixer.pragmas import parse_pragmas
...
def extract_mention(body: str | None, bot_login: str) -> str | None:
    ...
    part of the token, so `@autoawfixer-bot` does NOT match `@autoawfixer-bot-extra`).
    """
    ...
    *,
    ...
    if event_type == "pull_request" and action in ("opened", "reopened", "ready_for_review"):
        ...
        if pr_review_trigger == "vouched_label":
            # Defer to the vouch gate. autoawfixer reviews ONLY on the `labeled`
