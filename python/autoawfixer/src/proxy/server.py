...
Autoawfixer calls every endpoint with HMAC headers (see `autoawfixer.proxy_hmac`).
Authenticated requests dispatch to a single `GitHubClient` instance holding
the PAT, or to `autoawfixer.git_ops` for git transport. The PAT never leaves
this process.

Endpoint payloads are deliberately typed (no generic GitHub passthrough):
each one names exactly one operation autoawfixer performs.
"""
...
from autoawfixer.github_client import GitHubClient, GitHubError
from autoawfixer.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, verify
from autoawfixer.sandbox import _safe_directory_env, _slot_subprocess_kwargs
from autoawfixer.sandbox import workspace_key as compute_workspace_key
...
def _resolve_hmac_key(cfg: Settings) -> bytes:
    if cfg.gh_proxy_hmac_key is None:
        raise HTTPException(500, "gh-proxy: AUTOAWFIXER_GH_PROXY_HMAC_KEY not configured")
...
def create_proxy_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(title="autoawfixer-gh-proxy", version="0.1.0", lifespan=lifespan)
    ...
    # ---- git transport ----
    #
    # The underlying `autoawfixer.git_ops` primitives are blocking `subprocess.run`
