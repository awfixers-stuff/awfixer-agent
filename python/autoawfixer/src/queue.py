...
from autoawfixer.cancellation import clear_current_event, set_current_event
from autoawfixer.config import Settings
from autoawfixer.db import Database, EventRow
from autoawfixer.github_backend import GitHubBackend
from autoawfixer.sandbox import GitTransport, SandboxManager, _reap_slot
from autoawfixer.slot_pool import SlotPool
...
class WorkerPool:
    ...
    async def start(self) -> None:
        ...
        self._workers.append(asyncio.create_task(self._dispatch_loop(), name="autoawfixer-dispatch"))
        ...
        if self.sandbox.natives_cache is not None and self.settings.natives_cache_gc_interval_seconds > 0:
            self._workers.append(asyncio.create_task(self._natives_cache_gc_loop(), name="autoawfixer-natives-gc"))
    ...
    async def _dispatch_loop(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                task = asyncio.create_task(self._run_event(row), name=f"autoawfixer-event-{row.delivery_id[:8]}")
