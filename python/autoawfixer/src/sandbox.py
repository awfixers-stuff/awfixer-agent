...
PAT injection via `--config-env` (see `autoawfixer.git_ops`); the `ProxyGitTransport`
in `autoawfixer.proxy_client` forwards the same set of operations over HMAC RPC.
...
from autoawfixer.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `autoawfixer.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
