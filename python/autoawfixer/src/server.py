...
from autoawfixer import github_events
from autoawfixer.autoclose import AutocloseScheduler
from autoawfixer.config import Settings, get_settings
from autoawfixer.dashboard import render_index, static_dir, tail_jsonl
from autoawfixer.db import (
...
from autoawfixer.github_backend import GitHubBackend
from autoawfixer.github_client import GitHubError, IssueSummary
from autoawfixer.manual_triage import (
    InvalidIssueRef,
...
from autoawfixer.proxy_client import GitHubProxyClient, ProxyGitTransport
...
def _require_proxy_mode(cfg: Settings) -> tuple[str, bytes]:
    if cfg.github_token is not None:
        raise SystemExit(
            "autoawfixer orchestrator refuses to start with GITHUB_TOKEN set in env. "
            "The PAT must live only in the gh-proxy container."
        )
    if cfg.gh_proxy_url is None or cfg.gh_proxy_hmac_key is None:
        raise SystemExit(
            "autoawfixer orchestrator requires AUTOAWFIXER_GH_PROXY_URL and "
            "AUTOAWFIXER_GH_PROXY_HMAC_KEY (run gh-proxy in a sibling container)."
        )
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="autoawfixer", version="0.1.0", lifespan=lifespan)
    ...
    async def webhook(
        ...
        if decision.directive:
            ...
            payload["_autoawfixer_directive"] = {
    ...
    async def replay(
        request: Request,
        x_autoawfixer_token: str | None = Header(None, alias="X-Autoawfixer-Replay-Token"),
        delivery_id: str = "",
    ) -> JSONResponse:
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        ...
        if x_autoawfixer_token != cfg.replay_token.get_secret_value():
    ...
    def _require_trigger_token(cfg: Settings, token: str | None) -> None:
        if cfg.replay_token is None:
            raise HTTPException(404, "trigger disabled (set AUTOAWFIXER_REPLAY_TOKEN to enable)")
    ...
    async def api_github_issues(
        ...
        """Browse issues across `AUTOAWFIXER_REPO_ALLOWLIST` for the trigger picker.
        ...
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        _require_trigger_token(cfg, x_autoawfixer_token)
    ...
    async def api_trigger(
        ...
        if mode == "triage":
            ...
            if not cfg.allows(repo_full):
                raise HTTPException(403, f"{repo_full} not in AUTOAWFIXER_REPO_ALLOWLIST")
            try:
    ...
    async def api_logs(request: Request, limit: int = 400) -> dict[str, Any]:
        ...
        entries = tail_jsonl(cfg.log_dir / "autoawfixer.log.jsonl", limit=capped)
