...
from autoawfixer import persona
from autoawfixer.config import Settings
from autoawfixer.db import Database, IssueRow, IssueState, issue_key
...
from autoawfixer.sandbox import GitTransport, SandboxManager
from autoawfixer.worker import DirectiveInfo, TaskInputs, ThreadMessage, run_task
...
def _directive_from_payload(payload: Mapping[str, Any]) -> DirectiveInfo | None:
    ...
    raw = payload.get("_autoawfixer_directive")
...
async def handle_pr_conversation(
    *,
    ...
    # _autoawfixer_directive; _directive_from_payload rejects it but the key
    # being present tells us a mention happened. Reply cheaply without omp.
    if directive is None and payload.get("_autoawfixer_directive") is not None:
        comment = _comment_from_payload(payload)
        log.info(
