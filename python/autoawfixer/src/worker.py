...
from autoawfixer import host_tools, persona, pragmas
from autoawfixer.cancellation import register_cancel_hook, unregister_cancel_hook
from autoawfixer.config import Settings
from autoawfixer.db import Database, issue_key
from autoawfixer.git_ops import DirtyState, inspect_dirty_state
...
from autoawfixer.host_tools import AbortController, ToolBindings, _git_identity_env
from autoawfixer.natives_cache import NativesCache
from autoawfixer.natives_cache import compute_key as natives_compute_key
from autoawfixer.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    # `bash` tool could otherwise `printenv` them out of autoawfixer's env.
    "GITHUB_TOKEN",
    "GITHUB_WEBHOOK_SECRET",
    "AUTOAWFIXER_REPLAY_TOKEN",
    "AUTOAWFIXER_GH_PROXY_HMAC_KEY",
)
...
def _run_rpc_blocking(
    ...
    *,
    ...
    # can resolve workspace packages (@awfixerai/pi-*) and actually run tests.
