...
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>autoawfixer</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="autoawfixer-config" type="application/json">__AUTOAWFIXER_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "AUTOAWFIXER_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "AUTOAWFIXER_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "AUTOAWFIXER_BOT_LOGIN": "autoawfixer-bot",
        "AUTOAWFIXER_GIT_AUTHOR_NAME": "autoawfixer-bot",
        "AUTOAWFIXER_GIT_AUTHOR_EMAIL": "autoawfixer-bot@example.invalid",
        "AUTOAWFIXER_REPO_ALLOWLIST": "octo/widget",
        "AUTOAWFIXER_MODEL": "anthropic/claude-sonnet-4-5",
        "AUTOAWFIXER_THINKING": "high",
        "AUTOAWFIXER_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "AUTOAWFIXER_SQLITE_PATH": str(tmp_path / "autoawfixer.sqlite"),
        "AUTOAWFIXER_LOG_DIR": str(tmp_path / "logs"),
        ...
        # cache flip `AUTOAWFIXER_NATIVES_CACHE_ENABLED=true` explicitly.
        "AUTOAWFIXER_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "AUTOAWFIXER_NATIVES_CACHE_ENABLED": "false",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("AUTOAWFIXER_PROVIDER", raising=False)
    monkeypatch.setenv("AUTOAWFIXER_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("AUTOAWFIXER_GH_PROXY_URL", None)
    baseline.pop("AUTOAWFIXER_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("AUTOAWFIXER_GH_PROXY_URL", "")
    monkeypatch.setenv("AUTOAWFIXER_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("AUTOAWFIXER_PROVIDER", raising=False)
    monkeypatch.setenv("AUTOAWFIXER_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
