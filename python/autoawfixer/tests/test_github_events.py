...
from autoawfixer.github_events import (
    extract_mention,
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@autoawfixer-bot please re-review",
            },
            "issue": {"number": 9, "user": {"login": "contributor"}, "pull_request": {"url": "x"}},
            "repository": {"full_name": "octo/widget"},
        },
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @autoawfixer-bot please look", "autoawfixer-bot") == "hey please look"
    assert extract_mention("@autoawfixer-bot do X", "autoawfixer-bot") == "do X"
...
def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "autoawfixer-bot") is None
    assert extract_mention(None, "autoawfixer-bot") is None
    assert extract_mention("", "autoawfixer-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @AUTOAWFIXER-BOT", "autoawfixer-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @autoawfixer-bot-helper must NOT match @autoawfixer-bot.
    assert extract_mention("@autoawfixer-bot-helper hi", "autoawfixer-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@autoawfixer-bot one, then @autoawfixer-bot two", "autoawfixer-bot") == "one, then two"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@autoawfixer-bot go ahead and push",
            },
            "issue": {"number": 9},
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@autoawfixer-bot go ahead with the plan",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@autoawfixer-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@autoawfixer-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@autoawfixer-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
