...
from autoawfixer import host_tools
from autoawfixer.db import Database
from autoawfixer.github_client import GitHubClient, IssueInfo, RepoInfo
from autoawfixer.host_tools import AbortController, ToolBindings, build
from autoawfixer.sandbox import LocalGitTransport, Workspace
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    monkeypatch.setenv("AUTOAWFIXER_GH_PROXY_HMAC_KEY", "secret-proxy")
    ...
    assert env["AUTOAWFIXER_GH_PROXY_HMAC_KEY"] == ""
...
def test_guarded_push_branch_rev_parse_runs_via_repo_command_and_passes_slot_uid(
    ...
    from autoawfixer.git_ops import PushResult
    ...
    def fake_run_repo_command(
        ...
        if command[:3] == ["git", "log", "--format=%H%x09%ae%x09%an"]:
            return subprocess.CompletedProcess(
                command,
                0,
                "abc123\tautoawfixer-bot@example.invalid\tautoawfixer-bot\n",
                "",
            )
...
def test_gh_post_comment_happy_path(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 999, "user": {"login": "autoawfixer-bot"}, "body": "hi", "created_at": "t"})
...
def test_gh_post_comment_defaults_to_inbound_pr_thread(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        captured["url"] = str(request.url)
        return httpx.Response(201, json={"id": 7, "user": {"login": "autoawfixer-bot"}, "body": "hi", "created_at": "t"})
...
def test_repro_record_chowns_to_slot_when_root(db: Database, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "autoawfixer.host_tools.os.chown",
...
def test_mark_unable_posts_comment_marks_needs_info_and_labels_issue(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 321, "user": {"login": "autoawfixer-bot"}, "body": "x", "created_at": "t"})
...
def test_submit_pr_review_posts_summary_only_when_no_staged_comments(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"id": 45, "user": {"login": "autoawfixer-bot"}, "body": "ok", "state": "COMMENTED", "submitted_at": "t"},
        )
...
def test_gh_push_branch_rejects_wrong_identity(db: Database, tmp_path: Path) -> None:
    ...
    from autoawfixer.sandbox import SandboxManager
    ...
    try:
        ...
        assert "autoawfixer-bot <autoawfixer-bot@example.invalid>" in msg
...
def test_gh_push_branch_rejects_invalid_identity_scan_range(db: Database, tmp_path: Path) -> None:
    ...
    seed.mkdir()
    env = os.environ | {
        "GIT_AUTHOR_NAME": "autoawfixer-bot",
        "GIT_AUTHOR_EMAIL": "autoawfixer-bot@example.invalid",
        "GIT_COMMITTER_NAME": "autoawfixer-bot",
        "GIT_COMMITTER_EMAIL": "autoawfixer-bot@example.invalid",
    }
    subprocess.run(["git", "init", "--initial-branch=main", str(seed)], check=True, capture_output=True)
    (seed / "README.md").write_text("init\n")
    for cmd in (
        ...
        [
            ...
            str(seed),
            "-c",
            "user.email=autoawfixer-bot@example.invalid",
            "-c",
            "user.name=autoawfixer-bot",
            "commit",
            "-m",
            "init",
        ],
...
def test_gh_push_branch_runs_fix_and_check_before_pushing(
    ...
    assert lines[0].startswith("autoawfixer-bot <autoawfixer-bot@example.invalid> style: bun run fix"), lines
...
def test_gh_open_pr_runs_fix_then_check_and_commits_fixup(
    ...
    lines = log.stdout.strip().splitlines()
    assert lines[0] == "autoawfixer-bot|autoawfixer-bot@example.invalid|style: bun run fix"
...
def _stub_settings(*, enabled: bool = True, hours: float = 4.0):
    ...
    from autoawfixer.config import Settings
...
def _question_handler(captured: dict[str, Any]):
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201,
            json={"id": 4242, "user": {"login": "autoawfixer-bot"}, "body": "x", "created_at": "t"},
        )
