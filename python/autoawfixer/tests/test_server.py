...
from autoawfixer.dashboard import tail_jsonl
from autoawfixer.db import Database, close_database, get_database, issue_key
from autoawfixer.github_client import GitHubClient
from autoawfixer.manual_triage import InvalidIssueRef, ManualTriageTimeout, await_terminal_state, parse_issue_ref
from autoawfixer.sandbox import LocalGitTransport
from autoawfixer.server import create_app
...
def test_index_substitutes_replay_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AUTOAWFIXER_REPLAY_TOKEN", "secret-token-7")
...
def test_api_status_reports_runtime_counts_and_inflight(settings: Settings) -> None:
    ...
    assert runtime["bot_login"] == "autoawfixer-bot"
...
def test_api_logs_tails_jsonl_file(settings: Settings) -> None:
    log_path = settings.log_dir / "autoawfixer.log.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    payloads = [
        {"ts": "2026-05-14T21:28:28Z", "level": "INFO", "logger": "autoawfixer.queue", "msg": "dispatch loop online"},
        {
            "ts": "2026-05-14T21:28:54Z",
            "level": "INFO",
            "logger": "autoawfixer.server",
            "msg": "skip",
            "event": "issues",
            "reason": "issues.labeled ignored",
        },
        {"ts": "2026-05-14T21:30:00Z", "level": "WARNING", "logger": "autoawfixer.queue", "msg": "tool_end", "ok": False},
    ]
...
def test_parse_issue_ref_accepts_github_issue_urls() -> None:
    cases = (
        "https://github.com/awfixers-stuff/awfixer-agent/issues/1348",
        "http://github.com/awfixers-stuff/awfixer-agent/issues/1348",
        "github.com/awfixers-stuff/awfixer-agent/issues/1348",
        "https://www.github.com/awfixers-stuff/awfixer-agent/issues/1348",
        "https://github.com/awfixers-stuff/awfixer-agent/issues/1348/",
        "https://github.com/awfixers-stuff/awfixer-agent/issues/1348?foo=bar",
        "https://github.com/awfixers-stuff/awfixer-agent/issues/1348#issuecomment-99",
        "  https://github.com/awfixers-stuff/awfixer-agent/issues/1348  ",
    )
    for case in cases:
        assert parse_issue_ref(case) == ("awfixers-stuff/awfixer-agent", 1348), case
...
def _enable_replay(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("AUTOAWFIXER_REPLAY_TOKEN", token)
    reset_settings_cache()
    return token
...
def test_trigger_triage_fetches_and_enqueues(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.post(
            ...
            headers={"X-Autoawfixer-Replay-Token": token},
        )
    close_database()
...
def test_trigger_triage_rejects_repo_not_in_allowlist(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "AUTOAWFIXER_REPO_ALLOWLIST" in resp.json()["detail"]
...
def rate_limited_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> Settings:
    monkeypatch.setenv("AUTOAWFIXER_RATE_LIMIT_DEFAULT", "2")
    monkeypatch.setenv("AUTOAWFIXER_RATE_LIMIT_CONTRIBUTOR", "4")
    monkeypatch.setenv("AUTOAWFIXER_RATE_LIMIT_WINDOW_SECONDS", "3600")
    monkeypatch.setenv("AUTOAWFIXER_RATE_LIMIT_UNLIMITED", "can1357")
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
def _allowlist(monkeypatch: pytest.MonkeyPatch, repos: str) -> None:
    monkeypatch.setenv("AUTOAWFIXER_REPO_ALLOWLIST", repos)
    reset_settings_cache()
...
def test_browse_returns_401_with_replay_enabled_without_valid_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        wrong = client.get(
            "/api/github/issues",
            headers={"X-Autoawfixer-Replay-Token": f"{token}-wrong"},
        )
    close_database()
...
def test_webhook_directive_on_unknown_issue_is_queued_with_metadata(env, freeze_worker_pool) -> None:
    ...
    with TestClient(app) as client:
        resp = _post_issue_comment(
            ...
            user="can1357",
            number=77,
            body="@autoawfixer-bot please refactor X",
            association="OWNER",
        )
        assert resp.status_code == 202
    ...
    assert row is not None
    assert row.state == "queued"
    directive = row.payload.get("_autoawfixer_directive")
...
def test_webhook_directive_authorizes_deployed_app_login_without_author_association(
    ...
    monkeypatch.setenv("AUTOAWFIXER_BOT_LOGIN", "@roboomp[bot]")
    monkeypatch.setenv("AUTOAWFIXER_REPO_ALLOWLIST", "can1357/widget")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    app = create_app(cfg)
...
def test_webhook_maintainer_bypasses_rate_limit(
    ...
    """Login in AUTOAWFIXER_MAINTAINER_LOGINS is always unlimited, even with NONE association."""
    monkeypatch.setenv("AUTOAWFIXER_MAINTAINER_LOGINS", "can1357")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        ...
        for i in range(4):
            resp = _post_issue_comment(
                ...
                user="can1357",
                number=300 + i,
                body=("@autoawfixer-bot do X" if i == 3 else "comment"),
                association="NONE",
            )
            assert resp.status_code == 202
            states.append(resp.json()["state"])
    ...
    assert directive_event.payload.get("_autoawfixer_directive") == {
        "body": "do X",
        "author": "can1357",
        "pragmas": [],
...
def stub_run_task(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    from autoawfixer import tasks as tasks_module
...
async def test_handle_pr_conversation_unmapped_bot_pr_uses_pr_branch(
    ...
    from autoawfixer import tasks
    from autoawfixer.github_client import CommentInfo, IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_pr_conversation_skips_review_workspace_rows(
    ...
    from autoawfixer import tasks
    from autoawfixer.github_client import GitHubClient, PullRequestInfo
    ...
    payload = {
        ...
        "comment": {"user": {"login": "can1357"}, "body": "@autoawfixer-bot please re-review", "id": 12},
        "repository": {"full_name": "octo/widget"},
        "_autoawfixer_directive": {"body": "please re-review", "author": "can1357"},
    }
    await tasks.handle_pr_conversation(
        settings=settings,
        db=db,
...
async def test_review_pr_retries_when_ranked_but_not_submitted(
    ...
    from autoawfixer import tasks
    from autoawfixer.github_client import GitHubClient, IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_comment_directive_bootstraps_untriaged_issue(
    ...
    from autoawfixer import tasks
    from autoawfixer.github_client import GitHubClient, IssueInfo, RepoInfo
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_autoawfixer_directive": {"body": "please refactor X", "author": "can1357"},
    }
...
async def test_handle_comment_directive_reopens_finalized_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_autoawfixer_directive": {"body": "redo the fix", "author": "can1357"},
    }
    await tasks.handle_comment(
        settings=settings,
        db=db,
...
async def test_directive_handler_attaches_thread_from_github(
    ...
    from autoawfixer import tasks
    from autoawfixer.github_client import (
        CommentInfo,
        GitHubClient,
        IssueInfo,
        RepoInfo,
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_autoawfixer_directive": {"body": "do X", "author": "can1357"},
    }
...
async def test_triage_issue_skips_when_a_closing_pr_already_exists(
    ...
    from autoawfixer import tasks
    from autoawfixer.github_client import IssueInfo, RepoInfo
...
async def test_triage_issue_fails_open_when_timeline_fetch_errors(
    ...
    from autoawfixer import tasks
    from autoawfixer.github_client import GitHubError, IssueInfo, RepoInfo
