"""Contract test for the autoawfixer server API response vs status-contract.json.

Regenerate the committed fixture with:
    AUTOAWFIXER_UPDATE_STATUS_CONTRACT=1 pytest tests/test_status_contract.py
"""
...
from autoawfixer.db import get_database
...
_UPDATE_ENV = "AUTOAWFIXER_UPDATE_STATUS_CONTRACT"
...
def test_status_contract(settings: Settings) -> None:
    ...
    with TestClient(app) as client:
        ...
        # pytest runs assert equality; regenerate only when AUTOAWFIXER_UPDATE_STATUS_CONTRACT=1.
...
def test_cancel_errors_and_gating(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.post(
            "/api/cancel",
            json={"delivery_id": "run-cancel-2"},
            headers={"X-Autoawfixer-Replay-Token": "bad-token"},
        )
