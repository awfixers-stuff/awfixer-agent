...
These tests swap `autoawfixer.worker.RpcClient` for a recording fake so we can
...
from autoawfixer import worker
from autoawfixer.config import Settings
from autoawfixer.git_ops import DirtyState
...
def _patch_worker(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("autoawfixer.worker.RpcClient", _FakeRpcClient)
    monkeypatch.setattr("autoawfixer.worker._AGENT_HOME_STAGE", tmp_path / "missing-agent-home-stage")
    monkeypatch.setattr("autoawfixer.worker.host_tools.build", lambda _b: ())
    monkeypatch.setattr(
        "autoawfixer.worker.persona.system_append",
        lambda *, repo, issue, workspace, bot_login: "SYS",
    )
    monkeypatch.setattr(
        "autoawfixer.worker.persona.seed_phases",
...
async def test_run_rpc_omits_continue_when_session_empty(
    ...
    assert client_kwargs["env"]["AUTOAWFIXER_REPLAY_TOKEN"] == ""
    assert client_kwargs["env"]["AUTOAWFIXER_GH_PROXY_HMAC_KEY"] == ""
...
async def test_run_rpc_uses_workspace_xdg_dirs_for_slot_without_chown(
    ...
    monkeypatch.setattr("autoawfixer.sandbox.os.chown", lambda path, uid, gid: chown_calls.append((Path(path), uid, gid)))
...
async def test_run_rpc_arms_hard_timeout_timer(
    ...
    monkeypatch.setattr("autoawfixer.worker.threading.Timer", FakeTimer)
...
async def test_run_rpc_hard_timeout_stops_client_and_fails(
    ...
    monkeypatch.setattr("autoawfixer.worker.threading.Timer", FiringTimer)
    inputs, bindings = _make_inputs(tmp_path, settings, session_has_jsonl=False)
    loop = asyncio.new_event_loop()
    try:
...
async def test_run_rpc_cancel_hook_stops_and_marks_closed(
    ...
    captured: list = []
    monkeypatch.setattr("autoawfixer.worker.register_cancel_hook", lambda hook: captured.append(hook))
    monkeypatch.setattr("autoawfixer.worker.unregister_cancel_hook", lambda: None)
...
async def test_run_rpc_sends_reminder_when_pr_class_quits_early(tmp_path: Path, settings: Settings) -> None:
    ...
    try:
        ...
        loop.close()
    fake = _FakeRpcClient.instances[0]
    # kickoff + 2 reminders (default AUTOAWFIXER_TASK_COMPLETION_MAX_REMINDERS=2)
