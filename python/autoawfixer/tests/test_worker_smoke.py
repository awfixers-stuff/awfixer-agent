...
Runs only when AUTOAWFIXER_INTEGRATION=1 and `omp` is available on PATH (or via
AUTOAWFIXER_OMP_COMMAND). Spins up:
...
INTEGRATION = os.environ.get("AUTOAWFIXER_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="AUTOAWFIXER_INTEGRATION=1 required to run the omp-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from autoawfixer.sandbox import LocalGitTransport, SandboxManager
    from autoawfixer.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("AUTOAWFIXER_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("AUTOAWFIXER_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    ...
    monkeypatch.setenv("AUTOAWFIXER_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("AUTOAWFIXER_TASK_TIMEOUT_SECONDS", "300")
