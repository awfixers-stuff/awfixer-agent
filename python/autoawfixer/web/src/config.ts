...
// `__AUTOAWFIXER_CONFIG__` sentinel in `static/index.html` with a JSON blob so the
...
function readConfig(): AppConfig {
  const node = document.getElementById("autoawfixer-config");
  const text = node?.textContent?.trim();
  if (!text || text === "__AUTOAWFIXER_CONFIG__") {
...
export const AUTH_HEADERS: Readonly<Record<string, string>> = CONFIG.replayEnabled
  ? Object.freeze({ "X-Autoawfixer-Replay-Token": CONFIG.replayToken })
  : Object.freeze({});
