 ...
 * `AGENT_RELEASE_NOTES_FLOOR=v15.12.4` to override (empty string forces
 * single-version mode, matching the pre-#2596 behavior). `AGENT_REPO` /
...
const REPO = process.env.AGENT_REPO ?? process.env.GITHUB_REPOSITORY ?? "awfixers-stuff/awfixer-agent";
 ...
 *
 * Failure semantics:
 *   - `AGENT_RELEASE_NOTES_FLOOR` set → honored verbatim (`""` forces null).
 ...
 *     Local runs without `gh` should set `AGENT_RELEASE_NOTES_FLOOR=` to opt
 *     into legacy mode explicitly.
 */
async function resolvePublishedFloorTag(targetVersion: string): Promise<string | null> {
	const override = process.env.AGENT_RELEASE_NOTES_FLOOR;
	...
	if (res.exitCode !== 0) {
		...
		throw new Error(
			`gh release list exited ${res.exitCode}.\nstderr: ${stderr || "(empty)"}\n` +
				...
				`Locally without gh, set AGENT_RELEASE_NOTES_FLOOR= to fall back to single-version notes.`,
		);
	}
	let raw: unknown;
	try {
