...
const SUMS = {
	"agent-darwin-arm64": "darwin_arm64_sha",
	"agent-darwin-x64": "darwin_x64_sha",
	"agent-linux-arm64": "linux_arm64_sha",
	"agent-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["agent-*"].first` returns `nil`, and
	// `bin.install nil => "agent"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["agent-darwin-arm64", "agent-darwin-x64", "agent-linux-arm64", "agent-linux-x64"]) {
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"agent", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
