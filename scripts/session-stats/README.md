...
(`~/.agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.agent/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@awfixerai/ai`,
credentials resolved through agent's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.agent/stats-audit-cache.json` (keyed by session id +
